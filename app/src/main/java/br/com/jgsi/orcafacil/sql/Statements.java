...
public interface Statements extends Tabelas {
    ...
    String SELECT_ALL_ORCAMENTOS_POR_DATA = SELECT_ALL_ORCAMENTOS.replace(";", " ")
            + "WHERE 1 = 1 " +
            "AND (ORCAMENTO.data_inicio <=?1 AND ORCAMENTO.data_fim >=?2 );";
    String SELECT_ALL_ORCAMENTOS_POR_DATA_COM_INTERSECCOES = SELECT_ALL_ORCAMENTOS_POR_DATA.replace(";", " ").replace("AND (", "AND ((")
            + "OR (ORCAMENTO.data_inicio <=?1 AND ORCAMENTO.data_inicio >=?2 ) "
            + "OR (ORCAMENTO.data_fim >=?1 AND ORCAMENTO.data_fim <=?2 ) "
            + "OR (ORCAMENTO.data_fim >=?1 AND ORCAMENTO.data_inicio <=?2 )) ;";
    String SELECT_ORCAMENTO_BY_CATEG_DATA = SELECT_ALL_ORCAMENTOS_POR_DATA.replace(";", " ") + "AND CATEGORIA_DESPESA.id=?3 ;";
    String SELECT_ORCAMENTO_BY_CATEG_DATA_ORDENADO_DATA = SELECT_ORCAMENTO_BY_CATEG_DATA.replace(";", " ") + ORDER_BY_DATA_ORCAMENTO;
    String SELECT_ORCAMENTO_BY_CATEG_DATA_COM_INTERSECCOES = SELECT_ALL_ORCAMENTOS_POR_DATA_COM_INTERSECCOES.replace(";", " ") + "AND CATEGORIA_DESPESA.id=?3 ;";
    String SELECT_ORCAMENTO_BY_CATEG_DATA_COM_INTERSECCOES_ORDERNADO_DATA = SELECT_ORCAMENTO_BY_CATEG_DATA_COM_INTERSECCOES.replace(";", " ") + ORDER_BY_DATA_ORCAMENTO;
    String SELECT_ALL_ORCAMENTOS_POR_DATA_ORDENADO_DATA = SELECT_ALL_ORCAMENTOS_POR_DATA.replace(";", " ") + ORDER_BY_DATA_ORCAMENTO;
    String SELECT_ALL_ORCAMENTOS_POR_DATA_COM_INTERSECCOES_ORDENADO_DATA = SELECT_ALL_ORCAMENTOS_POR_DATA_COM_INTERSECCOES.replace(";", " ") + ORDER_BY_DATA_ORCAMENTO;
